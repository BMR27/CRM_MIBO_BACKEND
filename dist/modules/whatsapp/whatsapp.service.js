...
let WhatsappService = WhatsappService_1 = class WhatsappService {
    ...
    async downloadCloudMedia(mediaId, options) {
        ...
        if (!id) {
            throw new common_1.HttpException('mediaId is required', common_1.HttpStatus.BAD_REQUEST);
        }
        ...
        if (!downloadUrl) {
            throw new common_1.HttpException({ success: false, error: 'Cloud API did not return media url' }, common_1.HttpStatus.BAD_GATEWAY);
        }
    ...
    async uploadCloudMedia(input) {
        ...
        const BlobAny = globalThis.Blob;
        if (!FormDataAny || !BlobAny) {
            return {
                success: false,
                error: 'FormData/Blob not available in this runtime',
                hint: 'Actualiza Node a v18+ (recomendado v20+) para usar fetch/FormData/Blob nativos, o agrega un polyfill.',
            };
        }
    ...
    formatCloudApiError(data, context) {
        ...
        if (errorDetails)
            finalMessageParts.push(String(errorDetails));
        if (errorType)
            finalMessageParts.push(`type=${String(errorType)}`);
        return {
            success: false,
